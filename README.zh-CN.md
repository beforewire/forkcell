...
当前仓库是 `v0.1.0a2` preview（Python package version 为 `0.1.0a2`）。这个 preview 分支刻意保持精简：只包含 ForkCell 控制面、固定版本的 governed-runtime submodule、runtime patch provenance，以及足够让用户理解和跑通的文档/脚本。
...
当前 `0.1.0a2` 在 macOS + Docker Desktop 上的 preview validation：

| backend | 场景 | 文件数 | MiB | checkpoint | restore | full restore path | correctness |
|---|---|---:|---:|---:|---:|---:|---|
...
- `native-overlay` 仍然报告 `restore_sync_ms=0ms`，含义是同步 workspace generation switch sub-ms / 四舍五入为 0。
- `total_restore_path_ms` 包含 OpenShell sandbox lifecycle 和 delete；log collection 现在默认 best-effort non-blocking。需要等待 OCSF/log event 的 policy 测试可以加 `--sync-logs`。
- source preview 也验证了常见本地 secret 文件排除（`.env`、`.env.*`、`.ssh`、`.aws`、`*.pem`、`*.key`）以及 receipt/checkpoint policy binding。
...
`v0.1.0a2` 是实验性版本。这个 preview 用于展示产品边界，以及 checkpoint / restore / receipt 这条 working path。后续会继续补充更完整的运行时矩阵、验证门禁和示例场景。
